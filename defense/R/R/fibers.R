...
F1.stokes <- list(  PRE             = read.table(file="data/F1_stokes_pre.csv", header=T),
                    POST            = read.table(file="data/F1_stokes_post.csv", header=T),
                    change          = read.table(file="data/F1_stokes_change.csv", header=T),
                    POST.PREDICT    = read.table(file="data/F1_stokes_predict.csv", header=T),
                    PREDICT.ERROR   = read.table(file="data/F1_stokes_errorPredict.csv", header=T)  )
F1.error <- list(   PRE             = read.table(file="data/F1_error_pre.csv", header=T, row.names=1),
                    POST            = read.table(file="data/F1_error_post.csv", header=T, row.names=1),
                    change          = read.table(file="data/F1_error_change.csv", header=T, row.names=1) )
            
# Read data for optical fiber F2
F2.stokes <- list(  PRE             = read.table(file="data/F2_stokes_pre.csv", header=T),
                    POST            = read.table(file="data/F2_stokes_post.csv", header=T),
                    change          = read.table(file="data/F2_stokes_change.csv", header=T),
                    POST.PREDICT    = read.table(file="data/F2_stokes_predict.csv", header=T),
                    PREDICT.ERROR   = read.table(file="data/F2_stokes_errorPredict.csv", header=T)  )
F2.error <- list(   PRE             = read.table(file="data/F2_error_pre.csv", header=T, row.names=1),
                    POST            = read.table(file="data/F2_error_post.csv", header=T, row.names=1),
                    change          = read.table(file="data/F2_error_change.csv", header=T, row.names=1) )

# Read data for optical fiber F3
F3.stokes <- list(  PRE             = read.table(file="data/F3_stokes_pre.csv", header=T),
                    POST            = read.table(file="data/F3_stokes_post.csv", header=T),
                    change          = read.table(file="data/F3_stokes_change.csv", header=T),
                    POST.PREDICT    = read.table(file="data/F3_stokes_predict.csv", header=T),
                    PREDICT.ERROR   = read.table(file="data/F3_stokes_errorPredict.csv", header=T)  )
F3.error <- list(   PRE             = read.table(file="data/F3_error_pre.csv", header=T, row.names=1),
                    POST            = read.table(file="data/F3_error_post.csv", header=T, row.names=1),
                    change          = read.table(file="data/F3_error_change.csv", header=T, row.names=1) )
...
plot.polarisation <- function(data,
  ...
  ggplot(data = data.frame( W = c(data$POST$W, data$PRE$W) %>% as.factor,
    ...
    labs(title = title,
         x     = expression(bold("Rotation of Plane of Polarisation "*epsilon*" / °")),
...
plot.intensity.change <- function(data, 
  ...
  ggplot( data    = data$change, 
    ...
    theme(axis.text = element_text(size=12),
          ...
          panel.grid.minor.x = element_blank() ) +
    labs(title = title,
         x = expression(bold("Rotation of the Plane of Polarisation "*epsilon*" / °")),
         y = "Transmission / %" )
}
...
plot.stokesPredict <- function(data,
  ...
  grouped.data <- data.frame(
    ...
    Color = c( rep("Measured", length.out=length(data$POST$S0)), rep("Predicted", length.out=length(data$POST.PREDICT$S0)),
               rep("Measured", length.out=length(data$POST$S1)), rep("Predicted", length.out=length(data$POST.PREDICT$S1)),
               rep("Measured", length.out=length(data$POST$S2)), rep("Predicted", length.out=length(data$POST.PREDICT$S2)),
               rep("Measured", length.out=length(data$POST$polarisation)), rep("Predicted", length.out=length(data$POST.PREDICT$polarisation)) )
  )
  
  # Generate titles for the subplots
  facet.labels <- list( "S0" = "First Stokes-Parameter",
                        "S1" = "Second Stokes-Parameter",
                        "S2" = "Third Stokes-Parameter",
                        "polarisation" = "Degree of Polarisation" )
  ...
  ggplot(data = grouped.data, 
    ...
    labs(title = title,
         y = element_blank(),
         x = expression(bold("Rotation of the Half-Waveplate "*omega*" / °")),
         color = element_blank()) +
